...
async function createApplication(listingId, applicantId, applicationData) {
...
async function getListingApplications(listingId, requesterId) {
  ...
  const applications = await Applications.find({ listingId })
    .populate("applicantId", "jobseekerProfile.firstName jobseekerProfile.lastName")
    .sort({ createdAt: -1 });
...
async function updateApplicationStatus(applicationId, newStatus, requesterId) {
  const application = await Applications.findById(applicationId).populate("listingId");
