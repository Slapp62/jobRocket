...
const listingSlice = createSlice({
    ...
    reducers: {
        ...
        editListing: (state, action:PayloadAction<{listing:TListing}>) => {
            ...
            if (state.listings) {
                const index = state.listings?.findIndex((reduxListing) => reduxListing._id === updatedListing._id);
                    if (index !== -1){
        ...
        removeUserListing: (state, action:PayloadAction<TListing>) => {
          state.userListings = state.userListings?.filter((listing) =>
            listing._id !== action.payload._id)
        },
        addLike: (state, action:PayloadAction<{listing:TListing; userID:string}>) => {
            ...
            const listingIndex = state.listings.findIndex((reduxListing) => reduxListing._id === listing._id);
            if (listingIndex !== -1) {
